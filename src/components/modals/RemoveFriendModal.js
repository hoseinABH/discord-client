...
import { fake_friends as user } from 'utils/fake';
...
export default function RemoveFriendModal({ id, isOpen, onClose }) {
  ...
  const handleRemoveFriend = async () => {
    onClose();
    // const { data } = await removeFriend(fKey);
    // if (data) {
    //   queryClient.setQueryData(fKey, (friends) => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} isCentered>
      ...
      <ModalContent bg="brandGray.light">
        <ModalHeader
          ...
          pb={0}
        >
          Remove {user[0].username}
        </ModalHeader>
        <ModalBody>
          <Text mb={'4'}>
            Are you sure you want to permanently remove{' '}
            <b>{user[0].username}</b> from your friends?
          </Text>
        </ModalBody>
