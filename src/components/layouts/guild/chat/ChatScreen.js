...
// fake
import { fake_messages as messages } from 'utils/fake';
  ...
  const { data, isLoading } = useInfiniteQuery(
    ...
    },
    {
      getNextPageParam: (lastPage) =>
        hasMore && lastPage.length ? lastPage[lastPage - 1].createdAt : '',
    }
  );
      ...
      >
        {messages.map((m, i) => (
          <React.Fragment key={m.id}>
            <Message
              message={m}
              isCompact={checkIfWithinTime(m, [
                Math.min(i + 1, messages.length - 1),
              ])}
            />
            {checkNewDay(
              m.createdAt,
